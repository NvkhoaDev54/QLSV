...
public class EditSinhVienActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AppLogger.d(Constants.LOG_TAG_UI, "EditSinhVienActivity created");
    ...
    private void loadDataFromIntent() {
        originalMaSV = getIntent().getStringExtra(Constants.EXTRA_MA_SV);
        String hoTen = getIntent().getStringExtra(Constants.EXTRA_HO_TEN);
        String ngaySinh = getIntent().getStringExtra(Constants.EXTRA_NGAY_SINH);
        String gioiTinh = getIntent().getStringExtra(Constants.EXTRA_GIOI_TINH);
        String diaChi = getIntent().getStringExtra(Constants.EXTRA_DIA_CHI);
        String maLop = getIntent().getStringExtra(Constants.EXTRA_MA_LOP);

        etMaSV.setText(originalMaSV);
        etHoTen.setText(hoTen);
        etNgaySinh.setText(ngaySinh);
        etDiaChi.setText(diaChi);

        if (Constants.GENDER_MALE.equals(gioiTinh)) {
        ...
        if (maLop != null) {
            ArrayAdapter<String> adapter = (ArrayAdapter<String>) spinnerLop.getAdapter();
            for (int i = 0; i < adapter.getCount(); i++) {
                if (adapter.getItem(i).startsWith(maLop)) {
        ...
        AppLogger.d(Constants.LOG_TAG_UI, "Loaded data for editing: " + originalMaSV);
    }
    ...
    private void setupListeners() {
        ...
        btnUpdate.setOnClickListener(v -> updateSinhVien());
    ...
    private void showDatePicker() {
        ...
        DatePickerDialog datePickerDialog = new DatePickerDialog(
                EditSinhVienActivity.this,  // Hoặc EditSinhVienActivity.this
    ...
    private void updateSinhVien() {
        ...
        if (sinhVienService.updateSinhVien(sinhVien)) {
            AppLogger.d(Constants.LOG_TAG_UI, "Sinh vien updated successfully");
            Toast.makeText(this, Constants.MSG_UPDATE_SUCCESS, Toast.LENGTH_SHORT).show();
            finish();
        } else {
            Toast.makeText(this, Constants.MSG_UPDATE_FAILED, Toast.LENGTH_SHORT).show();
        }
    }

    private boolean validateInput(String hoTen) {
        if (hoTen.isEmpty()) {
